package algs.example.convexhull.parallel;
...
import algs.model.problems.convexhull.parallel.ConvexHullScan;
...
public class Main { 
	 ...
	 * 
	 * @param args   args[0] if it exists is integer representing number of threads to use.
	 */
	public static void main (String []args) {
		...
		for (int n = 2048; n <= 1048576; n*= 2) {  // = 131072
			...
			for (int t = 0; t < NUM_TRIALS; t++) {
				...
				IPoint[] hull1a = new ConvexHullScan(numThreads).compute(points);
				...
				System.gc();
				now = System.currentTimeMillis();
				IPoint[] reduced2a = AklToussaint.reduce(points);
				now2 = System.currentTimeMillis();
				IPoint[] hull2a = new ConvexHullScan(numThreads).compute(reduced2a);
				after = System.currentTimeMillis();
