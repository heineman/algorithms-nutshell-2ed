	...
	private boolean search(final int nodeId, int depth) {
		...
		while (st != null) {
			...
			if (exist == null) {
				...
				// record ID with this node for later and note we've been here.
				allIDS.insert(key, visited);
				prev.insert(key, visited);   
				
				// visit node
				visitor.visitNode(node, visited);
				visitor.visitEdge(here, visited);    // was nodeId, but that's too far removed...
